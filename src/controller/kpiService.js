...
const adminprocessKpidelete = async (req, res) => {
    try {
        // Extract the role and category name from request parameters
        const { role, categoryName } = req.params;

        // Ensure role is provided
        if (!role) {
            return res.status(400).json({ status: 400, success: false, error: "Role is required" });
        }

        // Define the query object based on the provided role and category name
        let query = { role };

        // If category name is provided, add it to the query
        if (categoryName) {
            query['processKpi.categoryName'] = categoryName;
        }

        // Delete KPI documents from the database based on the query
        const deleteResult = await databaseprocessKpimodel.deleteMany(query);

        // Check if any documents were deleted
        if (deleteResult.deletedCount === 0) {
            return res.status(404).json({ status: 404, success: false, error: "No matching data found for deletion" });
        }

        return res.status(200).json({ status: 200, success: true, message: "KPI data deleted successfully" });
    } catch (error) {
